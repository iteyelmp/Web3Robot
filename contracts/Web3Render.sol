...
contract Web3CuteRobot is Ownable {
    using strings for *;
    IW3RC3 public w3q;

    constructor(address contractData_) {
        setContractDataStorageAddress(contractData_);
    }
    ...
    function renderImage(bytes memory names) public view returns (bytes memory) {
        strings.slice memory s = string(names).toSlice();
        ...
        bytes memory access = bytes('');
        uint256 accSize = parts.length - 1;
        for (uint256 accIndex = 0; accIndex < accSize; accIndex++) {
            string memory name = parts[accIndex];
            (bytes memory acc,) = w3q.read(abi.encodePacked(name, '.png'));
            access = abi.encodePacked(
                access,
                '<image xlink:href="data:image/png;base64,', Base64.encode(acc), '"/>'
            );
        }

        string memory bg = parts[parts.length - 1];
        string memory idString = substring(string(bg), 0, bytes(bg).length - 4);
        (uint tokenId,) = stringToUint(idString);
